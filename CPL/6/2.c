...
int buf[BUFSIZE];
int bufp = 0;
char *ordertracker[2]; //Checks to see if the previous string has n characters similar
int  numCharsToCheck = 3;
...
//Prints out the tree, but groups similar starting words together
void treeprint(struct tnode *p)
{
    if(p != NULL) {
        ...
        int counter = 0;
        char *prevp, *currp; //Pointers to characters in previous and current words
        if(ordertracker[0] == NULL) {
            ordertracker[0] = p->word;
        }
        prevp = ordertracker[0];
        currp = p->word;
        
        while(counter++ < numCharsToCheck && *prevp++ == *currp++);
        if(counter > numCharsToCheck) {
            printf("%s\n", p->word);
        } else {
            printf("\n%s\n", p->word);   
        }
