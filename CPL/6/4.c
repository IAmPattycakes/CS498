...
int bufp = 0;
int linenumber = 1;
struct tnode *table[MAXOCCURRANCES];
struct tnode *currLargest;
int tablep = 0;
...
int main(void)
{
    ...
    while (treeprint(root) == 1) {
        printf("%4d %s\n", currLargest->count, currLargest->word);
        currLargest = NULL;
        tablep++;
    }
...
int treeprint(struct tnode *p)
{
    if(p != NULL) {
        ...
        } else if(currLargest->count < p->count) {
            int beenDoneBefore = 0;
            int i = 0;
            while(table[i] != NULL) {
                if(table[i] == p) {
...
int getword(char *word, int lim) 
{
    ...
    void ungetch(int);
    char *w = word;
    SPACESEARCH:while(isspace(c = getch()) && c != '\n'); //Skip over whitespace
